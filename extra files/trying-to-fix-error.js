...
function getUrl()
{
	...
	chrome.tabs.query({ //This method output active URL 
		...
		for (tab in tabs) {
			...
			var url1 = "https://www.raise.com/buy-"+company+"-gift-cards?type=electronic&page=1&per=200"
			/* if url1 is already at url2 then go to the deeper comment */
			...
			function loadXMLDoc() {
				...
				xmlhttp.onreadystatechange = function() {
					if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
						...
						var url1half = "https://www.raise.com/buy-gift-cards?utf8=%E2%9C%93&keywords="+company+"&type=electronic";

						var url2 = "";
						function loadXMLDoc1half() {
							var xmlhttp = new XMLHttpRequest();
							xmlhttp.onreadystatechange = function() {
								if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
									myFunction1half(xmlhttp);
								}
							};
							xmlhttp.open("GET", url1half, true);
							xmlhttp.send();
						}
						function myFunction1half(xml) {
							var x, i, xmlDoc, txt;
							xmlDoc = xml.responseText;
							parser = new DOMParser();
							xmlDoc = parser.parseFromString(xmlDoc,"text/html");
